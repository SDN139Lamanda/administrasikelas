...
export function validateInput(d) {
  const errors = [];
  if (!d?.sekolah) errors.push('Nama Sekolah wajib');
  if (!d?.jenjang) errors.push('Jenjang wajib');
  if (!d?.kelas) errors.push('Kelas wajib');
  if (!d?.semester) errors.push('Semester wajib');
  if (!d?.mapel) errors.push('Mapel wajib');
  if (!d?.topik) errors.push('Topik wajib');
