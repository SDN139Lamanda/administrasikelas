...
export async function renderCitaGenerator(jenjangFromParam, kelasFromParam, semesterFromParam) {
  ...
  if (!container) { console.error('❌ Container #cita-container not found!'); return; }
    ...
    <div class="cta-generator-form">
      ...
      <div id="cta-result" class="hidden result-section">
        ...
        <div style="display: flex; gap: 12px; margin-top: 16px;">
          ...
          <button type="button" id="btn-save" class="btn-save" style="flex: 1;"><i class="fas fa-save"></i> Simpan Manual</button>
...
// ✅ AUTO SAVE KE adm_pembelajaran - INI YANG KAMU MAU
async function autoSaveToAdmPembelajaran(result, metadata) {
  try {
    ...
    const docData = {
      ...
      userName: user.displayName || metadata.guru || 'Guru',
      nama_sekolah: metadata.sekolah,
      tahun_ajaran: metadata.tahun,
      ...
      jenis_dokumen: 'CP-TP-ATP',
      cp: result.cp,
      tp: result.tp,
      atp: result.atp,
      fase: getFase(metadata.kelas),
      mode: 'AI-Generated',
      status: 'draft',
      createdAt: serverTimestamp(),
      updatedAt: serverTimestamp(),
      // Metadata tambahan biar kompatibel sama dashboard adm
      jenjang_sekolah: metadata.jenjang,
      kelas_diampu: [metadata.kelas],
      mapel_diampu: [metadata.mapel],
      role: userData.role || 'teacher'
    };

    await addDoc(collection(db, 'adm_pembelajaran'), docData);
    console.log('✅ [CTA Generator] Auto-saved to adm_pembelajaran');
    return true;
  } catch(e) {
    console.warn('⚠️ Auto-save ke adm_pembelajaran gagal:', e);
    return null;
  }
}
...
console.log('🟢 [CTA Generator] READY — NO FILTER MODE + AUTO SAVE adm_pembelajaran');
