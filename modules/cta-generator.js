...
function filterCTAOptions(userData) {
  ...
  if (kelasSelect) {
    if (role === 'admin') {
      ...
      Array.from(kelasSelect.options).forEach(opt => opt.disabled = !['A', 'B'].includes(opt.value));
    } else if (['sd', 'mi'].includes(jenjang_sekolah) && sd_mapel_type?.toLowerCase() === 'kelas') {
      Array.from(kelasSelect.options).forEach(opt => opt.disabled = !kelas_diampu?.includes(opt.value));
    } else {
      const allClasses = jenjang_sekolah === 'sd' || jenjang_sekolah === 'mi' ? ['1','2','3','4','5','6'] :
                         jenjang_sekolah === 'smp' || jenjang_sekolah === 'mts' ? ['7','8','9'] : ['10','11','12'];
      Array.from(kelasSelect.options).forEach(opt => opt.disabled = !allClasses.includes(opt.value));
    }
  }
}
...
async function handleGenerate() {
  ...
  const aiReady = await isAiReady(); if (!aiReady) { alert('⚠️ AI belum aktif.'); return; }
...
async function handleSave() {
  ...
  try {
    ...
    await addDoc(collection(db, 'cp_tp_atp'), { 
      ...
      mode: 'AI', cp, tp, atp, createdAt: serverTimestamp(), updatedAt: serverTimestamp(), isAdmin: userDoc.exists() && userDoc.data()?.role === 'admin' 
    });
...
export async function autoSaveCTA(generatedContent, metadata) {
  try {
    ...
    const docData = { jenis: 'cta', jenjang: metadata.jenjang || '', kelas: metadata.kelas || '', mapel: metadata.mapel || '', judul: `CTA ${metadata.mapel || ''} Kelas ${metadata.kelas || ''}`, konten: generatedContent, tags: ['cta'], source: 'cta-generator' };
